  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {items.map((item, idx) => {
        ...
        const fileKey = item.src.split('/').pop()?.replace(/\.(jpg|jpeg|png|webp)/i, '') || '';
        const blurDataURL = IMAGE_BLURS[fileKey];

        return (
          <div 
            key={idx}
            className="bg-white rounded-xl border border-border-light overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group"
          >
            ...
            {/* Title & Desc */}
            <div className="p-5 border-t border-border-light/60 space-y-1 bg-white">
              <h3 className="font-display font-bold text-brand-primary text-sm">
                {item.title}
              </h3>
