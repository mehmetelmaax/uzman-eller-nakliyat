...
Bu tahmini hesaplama üzerinden detayları netleştirmek ve net teklif almak istiyorum.`;
  ...
  return (
    <div className="bg-white rounded-2xl border border-border-light shadow-xl overflow-hidden text-charcoal">
      {/* Top Banner */}
      ...
      {/* Steps Indicator */}
      <div className="flex justify-between items-center bg-surface-muted px-8 py-3 border-b border-border-light">
        <span className={`text-xs font-bold ${step >= 1 ? 'text-brand-accent-dark' : 'text-gray-400'}`}>1. Ev & Mesafe</span>
        <div className="h-0.5 bg-gray-300 flex-1 mx-4"></div>
        <span className={`text-xs font-bold ${step >= 2 ? 'text-brand-accent-dark' : 'text-gray-400'}`}>2. Kat & Asansör</span>
        <div className="h-0.5 bg-gray-300 flex-1 mx-4"></div>
        <span className={`text-xs font-bold ${step >= 3 ? 'text-brand-accent-dark' : 'text-gray-400'}`}>3. Sonuç & Teklif</span>
      </div>
      ...
      <div className="p-6 md:p-8">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-brand-primary uppercase block">Daire Tipi (Oda Sayısı)</label>
                <select
                  value={rooms}
                  onChange={(e) => setRooms(e.target.value as PriceInput['rooms'])}
                  className="w-full bg-surface-muted border border-border-light rounded-xl px-4 py-2.5 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-brand-accent/30 focus:border-brand-accent transition-all"
                >
              ...
              {/* Distance Selection */}
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-brand-primary uppercase block">Taşıma Güzergahı</label>
                <select
                  value={distanceType}
            ...
            {/* Storage Question */}
            <div className="flex items-start gap-3 bg-surface-muted p-4 rounded-xl border border-border-light/60">
              <input
                type="checkbox"
                id="storage"
                checked={storage}
                onChange={(e) => setStorage(e.target.checked)}
                className="w-4 h-4 mt-0.5 rounded text-brand-accent-dark focus:ring-brand-accent border-border-light cursor-pointer"
              />
              <div>
                <label htmlFor="storage" className="text-xs font-bold text-brand-primary block cursor-pointer select-none">
            ...
            <div className="flex justify-end pt-4">
              <button
                ...
                className="bg-brand-accent hover:bg-brand-primary text-brand-primary hover:text-white font-black px-6 py-2.5 rounded-xl border border-brand-primary transition-all duration-200 text-xs uppercase tracking-wider flex items-center gap-2 cursor-pointer active:scale-95"
              >
                <span>Devam Et</span>
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Origin Details */}
              <div className="space-y-4 bg-surface-muted p-5 rounded-xl border border-border-light/60">
                <span className="text-xs font-bold text-brand-accent-dark tracking-wider block">YÜKLEME ADRESİ (ÇIKIŞ KATİ)</span>
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-border-light rounded-xl px-4 py-2 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-brand-accent/30 focus:border-brand-accent transition-all"
                  />
                </div>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-4 h-4 rounded text-brand-accent-dark focus:ring-brand-accent border-border-light cursor-pointer"
                  />
                  <label htmlFor="fromElevator" className="text-xs font-bold text-brand-primary cursor-pointer select-none">
              ...
              {/* Destination Details */}
              <div className="space-y-4 bg-surface-muted p-5 rounded-xl border border-border-light/60">
                <span className="text-xs font-bold text-brand-accent-dark tracking-wider block">TESLİMAT ADRESİ (VARIŞ KATİ)</span>
                <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-4 h-4 rounded text-brand-accent-dark focus:ring-brand-accent border-border-light cursor-pointer"
                  />
                  <label htmlFor="toElevator" className="text-xs font-bold text-brand-primary cursor-pointer select-none">
                    Dış Cephe Mobil Asansör Kurulsun
                  </label>
                </div>
              </div>
        ...
        {step === 3 && estimate && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
              ...
              <div className="md:col-span-5 bg-brand-primary text-white rounded-2xl p-6 flex flex-col justify-center items-center text-center space-y-3">
                <span className="text-[10px] font-black text-brand-accent tracking-widest block">ÖNGÖRÜLEN BÜTÇE ARALIĞI</span>
              ...
              <div className="md:col-span-7 bg-surface-muted rounded-2xl p-5 border border-border-light flex flex-col justify-between">
                <div className="space-y-2">
                  <span className="text-xs font-bold text-brand-primary block">MALİYET KALEMLERİ:</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-border-light pt-4">
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  id="packingAddon"
                  ...
                  className="w-4 h-4 rounded text-brand-accent-dark focus:ring-brand-accent cursor-pointer"
                />
                <label htmlFor="packingAddon" className="text-xs font-bold text-brand-primary cursor-pointer select-none">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-4 h-4 rounded text-brand-accent-dark focus:ring-brand-accent cursor-pointer"
                />
                <label htmlFor="carpentryAddon" className="text-xs font-bold text-brand-primary cursor-pointer select-none">
            ...
            <div className="bg-brand-accent/10 border border-brand-accent/20 rounded-xl p-4 flex items-start gap-2.5">
              <CheckCircle2 className="w-5 h-5 text-brand-accent-dark shrink-0 mt-0.5" />
              <p className="text-[11px] text-brand-primary font-bold leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-2">
              ...
              <a
                ...
                className="bg-[#25D366] hover:bg-[#20ba5a] text-white font-black py-3 px-6 rounded-xl border border-brand-primary transition-all duration-200 text-xs flex items-center justify-center gap-2 flex-1 shadow-md active:scale-95 cursor-pointer"
              >
              ...
              <a
                ...
                className="bg-brand-primary hover:bg-brand-accent text-white hover:text-brand-primary font-black py-3 px-6 rounded-xl border border-brand-primary transition-all duration-200 text-xs flex items-center justify-center gap-2 flex-1 shadow-md active:scale-95 cursor-pointer"
              >
